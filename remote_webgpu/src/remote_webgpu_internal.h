...
typedef struct RemoteAdapter {
    ...
    /* Connected (websocket) fd to the remote GPU.  Owned by the adapter. */
    int socket_fd;
...
/* Serialize and send one envelope as a binary websocket message. */
int rw_send_envelope(int fd, const RemoteWebgpu__Envelope *envelope);

/*
 * Block until an envelope of `kind` arrives.  Notifications the client may
 * send at any time (CanvasResize) are absorbed into the adapter's state
 * along the way.  An Error envelope or a transport failure returns NULL.
 * Free the result with remote_webgpu__envelope__free_unpacked(env, NULL).
 */
...
/* The websocket fd behind a device. */
