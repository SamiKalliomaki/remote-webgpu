...
///
/// remote-webgpu: every record names the *main world* entity, because several
/// render worlds may exist and each maps main entities to its own render
/// entities.
...
pub(crate) enum EntityRecord {
    ...
    Added(Entity),
    /// When an entity is despawned on the main world, notify the render worlds so that the corresponding entities can
    /// be despawned. This contains the main world entity.
    Removed(Entity),
...
/// The pending sync records, one queue per registered render world.
///
/// remote-webgpu: upstream bevy keeps a single record queue because there is
/// exactly one render world.  Here every record is fanned out to each render
/// world's queue and consumed independently, so render worlds can be added
/// while the app runs and can skip frames without losing sync records.
#[derive(Resource, Default)]
pub struct PendingSyncEntity {
    /// One slot per registered render world; `None` after it unregisters.
    queues: Vec<Option<Vec<EntityRecord>>>,
}

impl PendingSyncEntity {
    pub(crate) fn push(&mut self, record: EntityRecord) {
...
/// The index of the render world's own queue in [`PendingSyncEntity`];
/// lives in each render world.
#[derive(Resource, Debug, Clone, Copy)]
pub struct SyncQueueIndex(pub usize);

/// This render world's mapping from main world entities to its own synced
/// entities; the multi-render-world replacement for reading [`RenderEntity`]
/// off the main entity.
#[derive(Resource, Default)]
pub struct MainToRenderEntityMap(pub bevy_ecs::entity::EntityHashMap<Entity>);

/// Registers a new render world with the main world's sync machinery and
/// returns the index of its record queue.  The queue starts with an `Added`
/// record for every currently synced entity, so render worlds created after
/// startup (a player joining mid-game) catch up on the existing scene.
pub fn register_render_world(main_world: &mut World) -> usize {
    let seed: Vec<EntityRecord> = main_world
        .query_filtered::<Entity, With<SyncToRenderWorld>>()
        .iter(main_world)
        .map(EntityRecord::Added)
        .collect();
    bevy_log::debug!("registering render world with {} seeded sync records", seed.len());
    let mut pending = main_world.resource_mut::<PendingSyncEntity>();
    pending.queues.push(Some(seed));
    pending.queues.len() - 1
}

/// Forgets a render world's record queue, e.g. when its player disconnects.
/// Indices of other render worlds stay valid.
pub fn unregister_render_world(main_world: &mut World, index: usize) {
    main_world.resource_mut::<PendingSyncEntity>().queues[index] = None;
}

pub(crate) fn entity_sync_system(main_world: &mut World, render_world: &mut World) {
    let index = render_world
        .get_resource::<SyncQueueIndex>()
        .map(|index| index.0)
        .unwrap_or(0);
    render_world.init_resource::<MainToRenderEntityMap>();
    main_world.resource_scope(|world, mut pending: Mut<PendingSyncEntity>| {
        render_world.resource_scope(|render_world, mut map: Mut<MainToRenderEntityMap>| {
            let map = &mut map.0;
            let Some(queue) = pending.queues.get_mut(index).and_then(Option::as_mut) else {
                return;
            };
                ...
                match record {
                    EntityRecord::Added(e) => {
                        if let Ok(mut main_entity) = world.get_entity_mut(e) {
                            ...
                            map.insert(e, id);
                            main_entity.insert(RenderEntity(id));
                        }
                    }
                    EntityRecord::Removed(main_entity) => {
                        if let Some(render_entity) = map.remove(&main_entity) {
                    ...
                    EntityRecord::ComponentRemoved(main_entity, removal_function) => {
                        let Some(&render_entity) = map.get(&main_entity) else {
                            continue;
                        };
                        if let Ok(render_world_entity) = render_world.get_entity_mut(render_entity)
                        {
            ...
            // Point each synced main entity's RenderEntity at *this* render
            // world; another render world may have extracted since we last
            // ran.  Cheap when nothing changed: a read plus an occasional
            // write.
            for (&main_entity, &render_entity) in map.iter() {
                if let Ok(mut entity_mut) = world.get_entity_mut(main_entity) {
                    match entity_mut.get::<RenderEntity>() {
                        Some(existing) if existing.0 == render_entity => {}
                        _ => {
                            entity_mut.insert(RenderEntity(render_entity));
                        }
                    }
                }
            }
