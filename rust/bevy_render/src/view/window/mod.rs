...
/// remote-webgpu: when present in a render world, that world extracts and
/// renders only this window.  Every per-player render world owns exactly one
/// window, because each window's surface lives on a different GPU (browser
/// tab) that only this world's device can touch.
#[derive(Resource, Debug, Clone, Copy)]
pub struct OwnedWindow(pub Entity);
...
fn extract_windows(
    ...
    owned: Option<Res<OwnedWindow>>,
) {
    for (entity, window, handle, primary) in windows.iter() {
        if owned.as_ref().is_some_and(|owned| owned.0 != entity) {
            continue;
        }
        if primary.is_some() {
