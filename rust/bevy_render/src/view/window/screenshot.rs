...
/// remote-webgpu: a harvested render world's screenshot sender points at its
/// throwaway app's (dropped) receiver; this re-points it at the same
/// receiver as `source`, an established render world.
pub fn share_screenshot_channel(source: &World, target: &mut World) {
    let sender = RenderScreenshotsSender(source.resource::<RenderScreenshotsSender>().0.clone());
    target.insert_resource(sender);
}
...
fn extract_screenshots(
    ...
    owned_window: Option<Res<super::OwnedWindow>>,
) {
    ...
    for (entity, screenshot) in screenshots.iter() {
        ...
        // remote-webgpu: leave screenshots of other render worlds' windows
        // alone, so the owning render world captures them.
        if let Some(owned) = owned_window.as_ref() {
            if let NormalizedRenderTarget::Window(window_ref) = &render_target {
                if window_ref.entity() != owned.0 {
                    continue;
                }
            }
        }
