...
pub(crate) fn extract_render_asset<A: RenderAsset>(
    ...
    // remote-webgpu: the asset-event cursor must be per render world (it was
    // a main-world resource upstream); a shared cursor would let the first
    // render world drain the events and starve every other one.
    mut local_state: Local<
        Option<
            SystemState<(
    ...
    {
        let world: &mut bevy_ecs::world::World = &mut main_world;
        if local_state.is_none() {
            *local_state = Some(SystemState::new(world));
        }
        let cached_state = local_state.as_mut().unwrap();
        {
            let (mut events, mut assets, maybe_render_assets) = cached_state.get_mut(world).unwrap();
