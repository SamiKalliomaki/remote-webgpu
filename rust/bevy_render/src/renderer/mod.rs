...
pub async fn initialize_renderer(
    ...
    // remote-webgpu: real window handles do not exist here, and automatic
    // renderer initialization is never used (each render world is created
    // with RenderCreation::Manual against one connected client), so the
    // compatible-surface hint is simply skipped.
