...
impl AssetServer {
    ...
    /// The number of [`AssetLoader`]s this server holds.
    ///
    /// FORK: exposed so that an app which builds the same plugin stack against
    /// one long-lived server over and over (see `register_loader`) can assert
    /// that doing so does not grow the loader table.
    pub fn registered_loader_count(&self) -> usize {
        self.read_loaders().len()
    }
