...
| `rust/` | Rust crates: a drop-in `wgpu` replacement backed by `remote_webgpu`, a `winit`-compatible event loop that runs the websocket server instead of opening a window, plus the FFI/runtime crates underneath (see `rust/README.md`).  Multiple clients can connect; each becomes its own `Window` with its own `Adapter`.  `./run_wgpu_example.sh <name>` runs any upstream wgpu demo against them; `cargo run -p bevy_game` (flat, hand-rolled renderer) and `cargo run -p bevy_pbr_game` (bevy's real PBR pipeline, via a vendored multi-render-world `bevy_render` fork) are example Bevy games where every connected tab is one player's view into the same world. |
