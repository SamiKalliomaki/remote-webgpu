...
| `rust/` | Rust crates: a drop-in `wgpu` replacement backed by `remote_webgpu`, a `winit`-compatible event loop that runs the websocket server instead of opening a window, plus the FFI/runtime crates underneath (see `rust/README.md`).  Multiple clients can connect; each becomes its own `Window` with its own `Adapter`.  `./run_wgpu_example.sh <name>` runs any upstream wgpu demo against them; `cargo run -p bevy_game` is an example Bevy game where every connected tab is one player's view into the same world. |
